...
app.config['MYSQL_DB'] = 'healabitdb'
...
#Recommandation (content-based algorithm)
df = pd.read_csv('recc2.csv')

df["Nutritions"] = df["Nutritions"].apply(lambda x: json.loads(x))
# my_list = ['spicy', 'maha', 'dal']
# l = {"Tags": [my_list]}
# df.append(pd.DataFrame(l), ignore_index=True)
...
tfv = TfidfVectorizer(min_df=3, max_features=None, strip_accents='unicode', analyzer='word', token_pattern=r'\w{1,}',
                      ngram_range=(1, 3), stop_words='english')       #TF id vectorizer, making matrix, and 
                                                         

df['bag'] = df['bag'].fillna('')

tfv_matrix = tfv.fit_transform(df['bag'])            #tansfer matraix and make sigmoid matrix into 
sig = sigmoid_kernel(tfv_matrix, tfv_matrix)         #445 vector matrix 
indices = pd.Series(df.index, index=df['recipies']).drop_duplicates()   #recommand according to recipies title
                                                    

#at first taking all data and showing directly in list
@app.route("/alldata")
def all_data():
    data = pd.DataFrame(
...
def index():
    ...
    return render_template("index.html")  # This assumes you have an "index.html" template in your templates folder
...
## BASED ON PROFILE                          #recommand according to the given data 
...
def get_final_data():
    if request.method == "POST":
        ...
        if data > 0:
            ...
            options = ast.literal_eval(l) # type: ignore
            try:
...
def get_healthy():
    if request.method == 'POST':
        ...
        if data > 0:
            ...
            glist = ast.literal_eval(l) # type: ignore
            rlist = ast.literal_eval(l2) # type: ignore
...
## RECOMENDATION             229 line no #jun recipies ma click garxa tei ko title 
...
def ing_list():
    if request.method == "POST":
        ...
        if data > 0:
            ...
            user_opt = ast.literal_eval(l) # type: ignore
            ...
            sorted_val = variable.sort_values(by=["length"], ascending=True) # type: ignore
